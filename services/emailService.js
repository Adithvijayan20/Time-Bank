...
const sendMailAndSMS = async (recepient_mail, name, otp, mailSubject, mailBody, smsBody) => {
    const transport = await mailer.createTransport(mailConfig);
...
const serviceStartMail = async (recepient_mail, name, otp) => {
    const mailSubject = "Time Bank - Your Volunteer is On the Way!";
...
<body>
  <div class="container">
    ...
    <div class="content">
      <h2>Hello ${name}! 😊</h2>
      <p>Your volunteer has accepted your service request and is on the way!</p>
      <p>Share this OTP before the service starts:</p>
      <div class="otp">${otp}</div>
    </div>
  </div>
</body>
    ...
    const smsBody = `Hello ${name}, your volunteer is on the way! Use OTP ${otp} to start the service.`;
    await sendMailAndSMS(recepient_mail, name, otp, mailSubject, mailBody, smsBody);
};

const serviceEndMail = async (recepient_mail, name, otp) => {
    const mailSubject = "Time Bank - Confirm Service Completion";
    const mailBody = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<body>
  <h2>Hello ${name}! 😊</h2>
  <p>Your service is complete. Use OTP ${otp} to confirm.</p>
</body>
</html>`;

    const smsBody = `Hello ${name}, your service has been completed. Use OTP ${otp} to confirm completion.`;
    await sendMailAndSMS(recepient_mail, name, otp, mailSubject, mailBody, smsBody);
};

module.exports = { serviceStartMail, serviceEndMail };
